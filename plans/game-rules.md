...
- Team: blue or red.
- Roles: spymaster, representative, guesser, observer.
...
- Observers do not occupy playable team seats and can chat only when observer chat is enabled.
...
- Commands with invalid card indexes, revealed cards, wrong teams, observers, or inactive guessers are rejected server-side without mutating state.
...
- All players and observers can see the active clue and finalized clue log.
...
- Non-spymaster players and observers see only revealed colors and remaining counts if useful.
- Observers always receive a non-spymaster view and can never enable spymaster view.
