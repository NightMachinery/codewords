...
func TestWebSocketForceThemeRequiresModAndBroadcastsSanitizedTheme(t *testing.T) {
	...
	if err := hostConn.WriteJSON(map[string]any{"type": "forceTheme", "theme": "glitch"}); err != nil {
		t.Fatalf("write host force glitch theme: %v", err)
	}
	if err := guestConn.ReadJSON(&msg); err != nil {
		t.Fatalf("read glitch theme broadcast: %v", err)
	}
	if msg["type"] != "themeForced" || msg["theme"] != "glitch" {
	...
	for _, theme := range []string{"christmas-cozy", "christmas-snow", "christmas-candy", "blood"} {
		...
		if err := guestConn.ReadJSON(&msg); err != nil {
			t.Fatalf("read %s theme broadcast: %v", theme, err)
		}
		if msg["type"] != "themeForced" || msg["theme"] != theme {
			t.Fatalf("expected %s theme broadcast, got %#v", theme, msg)
		}
	}
}
