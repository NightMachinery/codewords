...
<div class={['relative shrink-0', className].filter(Boolean).join(' ')}>
  ...
  {#if menuOpen}
    ...
    <div class="fixed right-3 top-16 z-[90] max-h-[min(calc(100dvh-5rem),32rem)] w-[calc(100vw-1.5rem)] max-w-72 overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900 p-3 text-left shadow-2xl shadow-slate-950/50 sm:right-8 sm:w-72" role="menu">
      <p class="px-1 pb-2 text-xs font-black uppercase tracking-[0.16em] text-slate-400">Theme</p>
      <label class="flex min-w-0 items-center gap-2 rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2 text-xs text-slate-300 cursor-pointer">
      ...
      {#if themePreferences.auto}
        <label class="mt-2 block min-w-0 text-xs text-slate-400">
          Dark mode theme
          <select class="mt-1 w-full max-w-full min-w-0 rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50" value={themePreferences.darkTheme} onchange={(event) => updateThemePreferences({ darkTheme: event.currentTarget.value as ThemeId })}>
        ...
        <label class="mt-2 block min-w-0 text-xs text-slate-400">
          Light mode theme
          <select class="mt-1 w-full max-w-full min-w-0 rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50" value={themePreferences.lightTheme} onchange={(event) => updateThemePreferences({ lightTheme: event.currentTarget.value as ThemeId })}>
        ...
        <div class="mt-2 grid gap-1.5" role="group" aria-label="Theme">
          {#each THEMES as theme (theme.id)}
            <button
              ...
              class={['flex min-w-0 items-center justify-between gap-2 rounded-xl border px-3 py-2 text-sm font-bold transition', effectiveThemeId === theme.id ? 'border-emerald-300/60 bg-emerald-300/10 text-emerald-100' : 'border-slate-700 bg-slate-950/60 text-slate-200 hover:border-emerald-300/40 hover:text-emerald-100'].join(' ')}
