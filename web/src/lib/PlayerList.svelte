<script lang="ts">
  import {
    ...
    canShowRoleControls,
    canShowTeamAssignmentButton,
    visiblePlayerBuckets,
    type LobbyPlayer,
    type Team
  } from './lobby';
  ...
  <article class={['group rounded-2xl border p-3 transition duration-300 hover:-translate-y-0.5 hover:border-slate-500', self ? 'border-emerald-300/70 bg-emerald-300/10 shadow-lg shadow-emerald-950/25' : 'border-slate-700 bg-slate-950/85'].join(' ')}>
    ...
    <div class="mt-3 flex flex-wrap gap-2">
      ...
      {#if canShowTeamAssignmentButton({ phase, mode, hostControls, player, viewer, team: 'red' })}
        <button class={['rounded-full border px-3 py-1.5 text-xs font-bold transition', player.team === 'red' ? 'text-slate-50' : 'text-slate-100/70']} style={`border-color: ${hexWithAlpha(teamColor('red', settings), player.team === 'red' ? 'cc' : '80')}; background-color: ${player.team === 'red' ? hexWithAlpha(teamColor('red', settings), '40') : 'transparent'};`} onclick={() => onAssignTeam(player.id, 'red')}>{displayTeamName('red', settings)}</button>
      {/if}
