<script lang="ts">
  ...
  import { chatScrollIsNearBottom, chatScrollShouldAutoScroll, chatToggleEventName, pressableButtonClasses } from './gameplay';
  ...
  let hasSavedScrollPosition = false;
  ...
  $effect(() => {
    ...
    const shouldAutoScroll = chatScrollShouldAutoScroll({
      expanded,
      messageCountIncreased,
      pendingOwnSend: pendingOwnSendScroll,
      wasNearBottom,
    });
    ...
    if (shouldAutoScroll) {
      void scrollToBottom();
    } else if (messageCountIncreased && pendingOwnSendScroll) {
  ...
  function updateScrollState() {
    ...
    wasNearBottom = chatScrollIsNearBottom(scrollContainer);
  }
  ...
  async function restoreScrollPosition() {
    ...
    if (hasSavedScrollPosition) {
      scrollContainer.scrollTop = Math.min(savedScrollTop, scrollContainer.scrollHeight);
...
<aside class="fixed bottom-24 right-0 top-0 z-40 flex w-80 flex-col border-b border-l border-slate-700/70 bg-slate-900/95 shadow-2xl transition-all duration-300">
  ...
  >
    ...
    <div bind:this={scrollContainer} onscroll={handleScroll} class="flex-1 overflow-y-auto p-4 space-y-3">
