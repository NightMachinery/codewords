<script lang="ts">
 ...
 ///
  ...
  let boardSurfaceHex = $derived(boardSurfaceHexForTheme(effectiveThemeId));
  ...
  let currentClue = $derived(currentSubmittedClue(activeClueLog));
  let visibleClueLog = $derived(visibleClueLogEntries(displayedClueLog));
  ...
  function emitUnitySpymasterCue() {
    if (preferences.clueVisualCue) {
      cueNotice = 'Your Unity board is live.';
      window.setTimeout(() => {
        if (cueNotice === 'Your Unity board is live.') cueNotice = '';
      }, 3600);
    }
    if (preferences.clueSound) {
      playUnitySpymasterCue();
    }
  }
  ...
  function playUnitySpymasterCue() {
    try {
      const AudioContextClass = window.AudioContext || (window as unknown as { webkitAudioContext?: typeof AudioContext }).webkitAudioContext;
      if (!AudioContextClass) return;
      const ctx = new AudioContextClass();
        ...
        <section class={activeMatchLayoutClasses()}>
          <div bind:this={boardContainer} class="space-y-6">
            ...
            {#if mode === 'monality' && hostControls && phase === 'active'}
              <button class={pressableButtonClasses('flex w-full items-center justify-center gap-2 rounded-2xl border border-purple-300/50 bg-purple-300/10 px-4 py-3 text-sm font-black uppercase tracking-[0.16em] hover:bg-purple-300/20')} style={`color: ${contrastAwareForeground(teamColor('monality', settings), boardSurfaceHex, 0.2)}`} onclick={closeMonalityRound}>Close Monality round</button>
            {/if}
                ...
                {#each visibleClueLog.slice().reverse() as clue, index (clueLogKey(clue, index))}
                  <article class="rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3">
                    <div class="flex items-center justify-between gap-3">
                      <span class={['rounded-full px-2.5 py-1 text-xs font-black capitalize', clue.team === 'blue' ? 'bg-blue-300 text-blue-950' : clue.team === 'unity' ? 'bg-teal-300 text-teal-950' : 'bg-red-300 text-red-950']} style={`background-color: ${teamColor(clue.team, settings)}; color: ${contrastAwareForeground(teamColor(clue.team, settings), boardSurfaceHex)}`}>{displayTeamName(clue.team, settings)}</span>
