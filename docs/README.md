...
- `frontend-theming.md` documents the app themes, including Dracula, Glitch, and Christmas homepage/board/card shaders, auto dark-mode switching, and moderator theme push.
...
- Pure Go game engine under `internal/game` for lobby roles, deterministic word boards, Polarity, Unity, and Monality modes, clue rounds, turn flow, hidden snapshots, and win conditions.
...
- JSON REST endpoints for identity, rooms, settings, match start, self and moderator target-player migrate links, wordpack listing, and picture catalog placeholders.
...
- Svelte/Tailwind frontend for browser identity bootstrap, display-name prompt, room create/join, lobby team/role controls, tabbed Polarity/Unity/Monality host settings, start-game action, room-link copy, self and moderator target-player migrate-device copy, active board play, Unity own-board review, Monality individual attempts, clue composer/log, guess/pass controls, observer read-only views, and game-over summaries and rankings.
