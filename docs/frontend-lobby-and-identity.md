...
- revealed card colors for all viewers, hidden-color tinting for spymasters, and all colors revealed after game over; word labels shrink or enlarge inside their cards and stay vertically centered, Latin-script labels that would overlap the top-left number badge shrink just enough to clear the badge band (reserved symmetrically so the label stays centered) while labels with room to spare use the full card height, Persian/Arabic labels keep the full-card fitting path, and only spaces or Persian half-spaces create deliberate wrap opportunities;
