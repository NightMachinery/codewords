...
- revealed card colors for all viewers, hidden-color tinting for spymasters, and all colors revealed after game over; word labels shrink or enlarge inside their cards, avoid the top-left number badge when badges are visible, use a glyph-safe line height for Persian/Arabic scripts, and only spaces or Persian half-spaces create deliberate wrap opportunities;
