...
The active board uses a continuous segmented remaining-count bar with SVG card-type icons. Image cards connect their color border directly to the image, last-selected image cards show the selected green border first and the card-color frame immediately inside it, and the greyed spymaster reveal style applies opacity 30 only to revealed cards instead of dimming every disabled card. Local settings can hide card number badges.
