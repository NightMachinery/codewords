...
Starting-lobby moderator settings support JSON5 setting profiles. Bundled defaults live in `assets/profiles/`, browser-saved profiles live in LocalStorage, and profile loading applies only known room setting fields so partial profiles are safe and extra fields are ignored. The bundled Vanilla profile is a 24-word manual-count board (`9` blue, `8` red, `7` neutral/civilian-plus-assassin with `1` assassin) so it differs from the 25-card classic defaults. Mid-game moderator settings hide lobby-only board-generation controls and keep only live/cosmetic controls plus round tools.
